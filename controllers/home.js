...
module.exports = {
  ...
  getProfile: async (req, res) => { // controls loading the logged-in user's profile
    try {
      ...
      let pets = await Pet.find({ client: req.user.id }); // Grabbing just the pets of the logged-in user 
      ...
      let visits = await Visit.find({ walkerSelect: req.user.id }).sort({ createdAt: 'desc' }).populate('client pet walkerSelect').lean(); // find all visit requests, sorted from oldest to newest (based on date created as of 7/26)
      console.log(user)
      console.log(visits)
